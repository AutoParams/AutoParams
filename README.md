...
> **Note**  
> The `freezeArgument` method relies on the availability of parameter names at runtime. However, Java does not include parameter names in bytecode by default. To ensure this works correctly, you can:
>
> 1. Use a record class, which preserves parameter names by design.
>
> 1. Compile with the `-parameters` option using javac.
> If you're using Spring Boot, this option is automatically enabled.
>
> 1. Apply the `@ConstructorProperties` annotation to explicitly declare parameter names.
